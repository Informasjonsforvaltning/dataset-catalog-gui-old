"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.getClassNames = void 0;

var _mergeStyles = require("@fluentui/merge-styles");

var _footerDekor = _interopRequireDefault(require("./assets/footerDekor.svg"));

var _skeLogo = _interopRequireDefault(require("./assets/ske-logo.svg"));

var _breakpoints = _interopRequireDefault(require("../utils/designtokens/_breakpoints.json"));

var _colors = _interopRequireDefault(require("../utils/designtokens/_colors.json"));

var _fontSizes = _interopRequireDefault(require("../utils/designtokens/_fontSizes.json"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function ownKeys(object, enumerableOnly) { var keys = Object.keys(object); if (Object.getOwnPropertySymbols) { var symbols = Object.getOwnPropertySymbols(object); enumerableOnly && (symbols = symbols.filter(function (sym) { return Object.getOwnPropertyDescriptor(object, sym).enumerable; })), keys.push.apply(keys, symbols); } return keys; }

function _objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = null != arguments[i] ? arguments[i] : {}; i % 2 ? ownKeys(Object(source), !0).forEach(function (key) { _defineProperty(target, key, source[key]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)) : ownKeys(Object(source)).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } return target; }

function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }

function setMinMaxWidth(props) {
  return {
    minWidth: props.minWidth,
    maxWidth: props.maxWidth
  };
}

function getMainBackgroundStyle(props) {
  if (props.layoutStyle === 'important') {
    return {
      background: `url(${_footerDekor.default})`,
      backgroundSize: 'calc(100% + 1px) 20px',
      backgroundRepeat: 'no-repeat',
      backgroundPosition: 'bottom left',
      paddingBottom: 30,
      backgroundColor: _colors.default['ske-color-white-100'],
      border: 'none !important'
    };
  } else {
    return {};
  }
}

function getHeaderBackgroundStyle(props) {
  if (props.layoutStyle === 'important') {
    const logoPlacement = props.title ? '-30px' : '-18px';
    return {
      backgroundImage: `url(${_skeLogo.default})`,
      backgroundSize: '40px 100%',
      backgroundRepeat: 'no-repeat',
      backgroundPosition: `top ${logoPlacement} left 20px`,
      paddingTop: 70
    };
  } else {
    return {};
  }
}

const getClassNames = function getClassNames(props, tag = 'SKE') {
  let primaryColor;

  switch (tag) {
    case 'INK':
      {
        primaryColor = _colors.default['ske-color-green-100'];
        break;
      }

    case 'LSO':
      {
        primaryColor = _colors.default['ske-color-black-100'];
        break;
      }

    default:
      {
        primaryColor = _colors.default['ske-color-burgundy-100'];
        break;
      }
  }

  const overflows = props.tabletContentOverflows;
  return (0, _mergeStyles.mergeStyleSets)({
    main: {
      displayName: `SkeDialog`,
      position: 'absolute',
      selectors: {
        '& .ms-Dialog-main': _objectSpread({
          height: overflows ? '100%' : '',
          selectors: {
            [`@media (min-width: ${_breakpoints.default['ske-breakpoint-sm']})`]: _objectSpread(_objectSpread({}, setMinMaxWidth(props)), {}, {
              borderColor: primaryColor,
              borderStyle: 'solid',
              borderWidth: '4px'
            })
          }
        }, getMainBackgroundStyle(props)),
        '& .ms-Modal-scrollableContent': {
          overflowY: 'auto',
          overflowX: 'hidden',
          WebkitOverflowScrolling: 'touch',
          selectors: {
            [`@media only screen and (min-device-width: 768px) and (max-device-width: ${'$ske-breakpoint-lg'})`]: {
              //iPad scrolling fix
              position: overflows ? 'absolute' : 'relative',
              top: 0,
              bottom: 0,
              minWidth: props.minWidth,
              maxWidth: props.maxWidth
            }
          }
        },
        '& .ms-Dialog-header': _objectSpread({}, getHeaderBackgroundStyle(props)),
        '& .ms-Button.ms-Dialog-button--close i': {
          fontWeight: _fontSizes.default['ske-font-weight-bold'],
          fontSize: _fontSizes.default['ske-font-size-icon-l'],
          opacity: 1
        },
        '& .ms-Button.ms-Dialog-button--close:hover i': {
          fontWeight: _fontSizes.default['ske-font-weight-bold'],
          fontSize: _fontSizes.default['ske-font-size-icon-l'],
          opacity: 0.7,
          transition: 'opacity 300ms ease-out'
        },
        '& .ms-Dialog-title': {
          fontSize: _fontSizes.default['ske-font-size-xl'],
          fontWeight: _fontSizes.default['ske-font-weight-semibold'],
          padding: props.layoutStyle === 'airy' ? '128px 64px 0 64px' : '20px 50px 20px 20px'
        },
        '& .ms-Dialog-subText': {
          fontSize: _fontSizes.default['ske-font-size-m']
        },
        '& .ms-Overlay': {
          backgroundColor: 'rgba(255, 255, 255, 0.8)'
        },
        '& .ms-Dialog-inner': {
          padding: props.layoutStyle === 'airy' ? '0 64px 64px ' : '0 20px 20px 20px'
        },
        '& .wait-alert-btn': {
          width: '100%'
        },
        '& .wait-alert-content': {
          overflowX: 'hidden',
          overflowY: 'hidden',
          maxWidth: '100%',
          maxHeight: '100%'
        },
        '& .wait-alert-heading': {
          fontSize: _fontSizes.default['ske-font-size-xl'],
          fontWeight: 'bold'
        },
        '& .wait-alert-svg': {
          width: '290px'
        }
      }
    },
    helpButton: {
      displayName: 'SkeHelpButton',
      zIndex: 100,
      position: 'absolute',
      bottom: '-60px',
      left: '-15px'
    }
  });
};

exports.getClassNames = getClassNames;