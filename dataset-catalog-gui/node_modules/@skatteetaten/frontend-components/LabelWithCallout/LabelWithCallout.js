...
import { calloutState } from './LabelWithCallout.types';
/*
 * visibleName LabelWithCallout (Merkelapp med utropsboks)
 */

export const LabelWithCallout = props => {
  const {
    id,
    inputId,
    label,
  ...
  const styles = getClassNames(_objectSpread({
    calloutFloating
  }, props));
  const [isCalloutVisible, setIsCalloutVisible] = React.useState(false);
  const [currentCalloutState, setCurrentCalloutState] = React.useState(calloutState.CLOSED);
  ...
  const toggleEvent = () => {
    if (onCalloutToggle) {
      ...
      const newCalloutState = isCalloutVisible ? calloutState.CLOSED : calloutState.OPEN;
  ...
  }))), isCalloutVisible && /*#__PURE__*/React.createElement(Callout, _extends({}, calloutProps, {
